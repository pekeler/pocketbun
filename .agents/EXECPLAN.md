...
- [ ] Finish the corrected Linux scaling matrix before judging cluster efficiency. Its first target-300/FIFO round changed PocketBun's one-to-two-worker 150-scenario speedup from the obsolete matrix's 1.05x to 1.59x, with aggregate request shares of 48.08%/51.92%. The 112 read scenarios improved 1.82x, matching PocketBase's earlier 1.81x; expensive concurrent reads and high-concurrency routes commonly improved 1.9x-2.12x. Preserve five-run medians and the pending four-worker/process-CPU evidence before closing the gate.
