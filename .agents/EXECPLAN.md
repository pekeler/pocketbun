...
- [ ] Complete the post-fix release qualification before accepting new performance evidence. The focused transaction/protocol/realtime suites, a real two-worker five-delete batch with an injected preparation failure, all ten real-process cluster lifecycle tests, format, typecheck, lint, and the complete 1,964-test local concurrent suite pass. Hosted Ubuntu/macOS/Windows CI, a current-revision cluster soak, and the focused Linux cascade reproduction remain.
- [ ] Rerun the upstream 150-scenario PocketBase benchmark workload internally with one, two, and four PocketBun workers only as a compatibility/regression suite. The exact-`8048583e` attempt rejected an omitted ignored JavaScript hook fixture, and the exact-`d1e22db1`/`1d4017ac` attempts exposed cluster correctness issues. None of the partial datasets is publishable; require five complete zero-error runs per worker count with exact source provenance.
- [ ] Build the public PocketBase/PocketBun performance matrix around one identical external load generator on a separate host. Run PocketBase and PocketBun one/two/four-worker configurations against the same application host, data, scenarios, concurrency, warm-up, and five-run median policy; record throughput, p50/p95/p99, errors, application CPU/RSS, and load-generator CPU/network headroom. Keep the upstream co-located suite clearly labeled and do not use it as cluster-scaling evidence.
...
Milestone 10's production-correctness gate is complete. Hosted Ubuntu, macOS, Windows, and E2E runs qualify leader and follower startup failures, transaction rollback, pending and lost coordinator responses, primary death, protocol violations, crash-budget exhaustion, SQLite contention, backup ownership, restart, supported-platform restore, SSE-owner replacement, and adversarial cross-client realtime isolation. Windows built-in restore is intentionally unsupported exactly like PocketBase v0.40 and its unchanged Admin UI. Stateful one-hour Linux soaks passed with two and four workers, later v0.40 two-worker and four-worker five-hour fault soaks completed every functional and resource-integrity assertion, and the final fixed-PID two-hour soak found no serious or operationally unbounded worker or primary retention. PocketBase v0.40's live-backup boundary is ported and cluster-qualified, including distinct owner/deleter/uploader processes and exact archive file inclusion. Read scaling reached +65.7% across two physical Linux cores, the DB-free control reached +94.6%, and the local macOS proxy topology scaled monotonically through eight workers. Primary-pressure testing then found an individual-IPC rate-limit bottleneck; ordered batching raised the four-worker path from 1,238 to 27,980 req/s while preserving exact primary-owned decisions. The matched realtime run delivered all 672,320 expected events exactly and saturated workers before the primary. Production documentation and the post-batching local repository, package, docs, mapping, Admin UI, and current hosted CI gates are complete.

No known production-correctness issue remains open. Release work is now limited to completing the reproducible one/two/four-worker benchmark matrix, publishing its results without selectively dropping regressions, and final release documentation/version preparation. The first matrix attempt usefully caught an omitted ignored benchmark fixture before invalid data was accepted; it did not expose a PocketBun runtime failure. Windows built-in restore is an accepted compatibility exclusion, and modest RSS fluctuations remain a production-monitoring observation rather than separate implementation work. No unexplained flaky result may be waived by adding a default retry.
