...
- [ ] Qualify ordered realtime presence on the exact corrected revision: pass the deterministic ordering regression, the complete local gate and hosted CI, an accelerated four-worker controlled-restart soak, then fresh two- and four-worker fault soaks of at least 60 minutes with no steady-state or controlled-restart event loss.
