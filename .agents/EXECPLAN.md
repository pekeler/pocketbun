...
- [ ] Publish the external PocketBase/PocketBun performance matrix only after the soak blocker is closed and the benchmark caveats are resolved. The identical two-vCPU external driver executed all 154 timed batches per run, including the four background-write batches, with zero load errors and p95 client CPU below 73%. Five-run scenario medians show PocketBun one/two/four-worker summed Completed times of 519.8/494.6/468.2 seconds versus PocketBase's 699.1 seconds; high-concurrency routes and simple list reads scale about 1.74–1.93x at two workers and 1.89–2.44x at four. Whole-suite scaling is only 1.05x/1.11x because low-concurrency and SQLite-bound scenarios dominate. Four-worker repeatability is materially noisier (21.9% total range around its median; 68/150 scenarios have a greater-than-20% range), the upstream result format has no p50/p95/p99, and network headroom was not recorded, so do not yet turn these into README claims.
