...
- Decision: use an external load-generator host for public PocketBase/PocketBun performance and scaling results.
  ...
  Date/Author: 2026-08-24 / repository owner and Codex
- Decision: treat every matched organization-create regression as a release performance bug and require a repeatable margin, not a one-run numerical win.
  Rationale: the repository owner wants PocketBun faster across all published scenarios. Milestone 11 therefore accepts both organization scenarios only when their five-run medians are at least 5% below PocketBase at one, two, and four application lanes on the same external topology. The exact 50-request, concurrency-10 workload remains un-warmed because changing it would hide rather than improve the measured cold burst; a separate warmed run guards steady-state throughput.
...
- Exact fresh-process and warmed organization measurements are both recorded. No hidden warm-up is added to the upstream comparison, and the focused helper waits the same two seconds after collection changes before attributing time to the request path.
