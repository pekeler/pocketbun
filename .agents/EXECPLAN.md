...
- [ ] Design and implement Windows built-in backup restore. PocketBase's public documentation presents restore without a platform caveat, but its Unix-only `execve` implementation rejects Windows; PocketBun inherited that guard and currently only verifies PocketBase-archive extraction on Windows. Do not treat the rejection as a supported PocketBun contract: decide and qualify a safe close/replace/relaunch handoff that preserves rollback and cluster behavior, then run the real fixture restore on Windows.
...
- [ ] A later four-worker five-hour run against the pre-v0.40 tree completed functionally: 302,244 CRUD/SQLite cycles, four realtime clients, 150 verified backups, repeated worker replacement and full restarts, stable 89–95 descriptors, final count/checksum verification, clean shutdown, and no logged errors. Worker RSS remained stable, but primary RSS hourly medians rose from 76.4 to 110.1 MiB (about 8.5 MiB/hour). Record this only as an unresolved primary-RSS trend, not a confirmed memory leak: earlier focused investigations showed that apparent RSS growth can be allocator/cache warm-up rather than retained heap. Defer another memory investigation until the other release work is complete. This run predates the v0.40 live-backup coordination changes and therefore does not replace the remaining current-revision one-hour soak.
...
- [ ] (2026-08-24) Hosted run `32669876437` passed macOS and Windows but the Ubuntu coordination test timed out at its initial three-worker state check. All three workers had already reported ready; the Linux helper then made untargeted requests through one `reusePort` listener and discarded each incomplete 30-request sample. It now excludes each observed PID while collecting the remaining workers and reports the last states on a genuine convergence timeout. Four focused local runs and the complete 1,958-test gate pass; rerun the hosted matrix before closing this item.
- [ ] Run the current revision through the full cross-platform CI/manual workflows, the final one-hour cluster soak, and the no-known-correctness-failure release review.
...
Milestone 10 remains the production-release gate, but most of its evidence is now complete. Hosted Ubuntu, macOS, and Windows runs qualify leader and follower startup failures, transaction rollback, pending and lost coordinator responses, primary death, protocol violations, crash-budget exhaustion, SQLite contention, backup ownership, restart, restore, SSE-owner replacement, and adversarial cross-client realtime isolation. Stateful one-hour Linux soaks passed with two and four workers on the pre-v0.40 tree; the later five-hour run also completed every functional and resource-integrity assertion, but its unresolved primary-RSS trend is recorded only as an observation and not as a confirmed leak. PocketBase v0.40's live-backup boundary is ported and cluster-qualified locally, including distinct owner/deleter/uploader processes and exact archive file inclusion. Read scaling reached +65.7% across two physical Linux cores, the DB-free control reached +94.6%, and the local macOS proxy topology scaled monotonically through eight workers. Primary-pressure testing then found an individual-IPC rate-limit bottleneck; ordered batching raised the four-worker path from 1,238 to 27,980 req/s while preserving exact primary-owned decisions. The matched realtime run delivered all 672,320 expected events exactly and saturated workers before the primary. Production documentation and the post-batching local repository, package, docs, mapping, and Admin UI gates are complete.

Release is now blocked on hosted Ubuntu/macOS/Windows and E2E qualification of the current revision, one final current-revision one-hour cluster soak, and the no-known-correctness-failure review. The public PocketBase/PocketBun benchmark matrix remains intentionally deferred until those correctness gates pass. Windows built-in restore and the unconfirmed primary-RSS trend remain separately recorded follow-up work and must not distract from the current gates. No unexplained flaky result may be waived by adding a default retry.
...
PocketBun currently rejects backup restore on Windows because it inherited PocketBase's Unix-only process-replacement implementation. This is not an accepted PocketBun capability: defer the separate Windows close/replace/relaunch design until cluster hardening is otherwise complete, then qualify real restore rather than the rejection. Until then, test cluster-wide restore on Linux and macOS and keep Windows coverage focused on archive extraction and continued cluster health.
...
Revision note, 2026-08-22 / Codex: Closed Milestone 9 after corrected hosted run 32568697758 passed the complete Ubuntu, macOS, Windows, and downstream Playwright E2E matrix at commit `bb15bad4`. Cluster-wide backup exclusion, owner-death recovery, restart, and supported-platform restore are qualified. The inherited Windows restore rejection is not an accepted PocketBun capability and remains a separate pending design/qualification item; Milestone 10 is the remaining hardening, performance, documentation, and release gate.
