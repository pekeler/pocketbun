...
- [ ] Validate the revised symmetric default on Linux: cap long warmup scenarios at 300 requests, repeat the short organization and permission create batches until each variant has received at least 300 requests, then clear all disposable data and run the measured suite. Confirm zero errors, balanced FIFO worker participation, improved short-create rows, and no broad regression for both PocketBun and PocketBase before accepting the final five-run matrix. The first matched four-worker run made PocketBun's organization rows 8.5% and 7.4% faster than PocketBase (17.36/17.56 versus 18.97/18.96 ms), while 150 had left the no-rule row 27% slower; four more alternating repetitions are required.
