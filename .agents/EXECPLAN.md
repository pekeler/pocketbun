...
The benchmark name “Creating organizations” means creating ordinary records in the benchmark's `organizations` collection; it does not mean changing PocketBase collection schemas. Each of its two scenarios sends 50 unauthenticated JSON `POST /api/collections/organizations/records` requests with ten requests in flight. The first collection has an empty create rule and the second evaluates `@request.body.name != ''`. Before every timed burst, the benchmark updates the rule, counts existing records, and waits two seconds. The accepted comparison uses one external load-generator host, one fixed application host, five runs, and matched PocketBase `GOMAXPROCS=1/2/4` versus PocketBun one/two/four workers. Both targets first run the same selected scenarios untimed with a 100-request cap, after which the measured `create` phase clears warmup data without restarting the application.

Before accepting 100 as the final cap, run a focused four-worker tiering curve for both organization variants. Record the serving worker slot for every warmup request and expose benchmark-only `bun:jsc.numberOfDFGCompiles()` diagnostics for a small fixed set of actual record-create, rule-evaluation and response-path functions in every worker. Compare fresh-process warmup levels of 50, 150, 300 and 600 requests per variant, with five short repetitions at each level, and stop when the measured median and per-worker DFG state plateau. Treat those counts as record-create/rule-evaluation requests, not collection-rule updates: the collection update itself runs once in the benchmark-controller worker and is outside the timed request burst. Do not attempt to force FTL everywhere; its normal threshold is orders of magnitude higher, and a public steady-state benchmark should not spend hours manufacturing a tier that the workload does not naturally reach. A total of 150 requests is only 37.5 per worker under perfect distribution, so it is a baseline-JIT hypothesis rather than a safe default.
