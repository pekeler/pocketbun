...
- [ ] (2026-08-22 13:00Z) Added a real-process forced-shutdown case in which a follower enters but never completes `onTerminate`; the primary reaches its existing ten-second deadline, kills the follower, and exits without leaving any worker PID. The focused file passes in 12.60 seconds, the complete 1,936-test concurrent suite passes in 83.86 seconds, the four-process suite passes in 32.17 seconds, and typecheck/lint are clean; hosted qualification is pending.
