...
- [ ] (2026-08-22 15:31Z) Added local real-process coverage for both leader and follower never-ready crash-budget exhaustion plus malformed protocol version, duplicate-ready, and late-result IPC. Every offending worker is removed and replaced in the same role/slot, and the no-ready cases leave no worker or ownership guard. Focused and complete local qualification pass; hosted confirmation remains required.
...
Milestone 10 is the remaining production-release gate. Functional compatibility and the existing real-process cluster tests provide a strong base, but they do not yet qualify prolonged resource stability under production-like traffic. Hosted CI now qualifies the leader migration transaction, post-commit/pre-ready, post-ready, follower transaction, follower pending-primary-response, and primary-death coordinator boundaries. The follower transaction rolls back completely and later writes succeed. An already-delivered idempotent subscription update may finish after its source dies even though the source HTTP request rejects, and the replacement can repeat it safely. Primary death after completing rate-limit, realtime-prepare, OAuth2, backup-acquire, and restore-begin work is also qualified: source requests disconnect, workers terminate with their IPC parent, and a new real primary recovers the stale guard and has no inherited transient coordinator state. The repeated Windows final-guard failure was an invalid test assumption: Bun force-terminates spawned Windows children for `kill("SIGTERM")`, so no JavaScript cleanup can run and a stale guard is the correct result. Windows pins that forced-death invariant while POSIX separately pins graceful guard removal; the speculative heartbeat serialization is gone. The formerly failing Windows backup scenario and the corrected guard behavior passed hosted run 32581531550. Leader/follower no-ready crash exhaustion and malformed, duplicate-ready, and late-result IPC cleanup pass locally and await hosted confirmation. Shared-database pressure then exposed a real Linux locking-protocol failure in transaction-wrapped live-file backups; Bun SQLite serialization snapshots now keep manual and automatic archives coherent while other workers write and checkpoint. Exact final counts/checksums, interrupted-writer rollback, archive integrity, later backup, restart, and restore pass locally and across 20 clean Ubuntu repetitions; hosted cross-platform confirmation remains. Graceful systemd shutdown, benchmark-app termination, and the clean clustered public collection-update path are also corrected and covered. Release remains blocked on hosted qualification of the latest fault and SQLite batch, successful 60-minute two-worker and four-worker Linux soaks, primary saturation measurements, matched current PocketBase/PocketBun benchmarks, operational documentation, and the complete repository and hosted gates. No unexplained flaky result may be waived by adding a default retry.
