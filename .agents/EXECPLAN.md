...
- [ ] (2026-08-22 14:30Z) Hosted run 32577240713 passed Ubuntu and macOS but failed two real Windows lifecycle boundaries without retries: the shared-state scenario's held backup returned HTTP 400 after a competing settings write, and the primary-death matrix left the final ownership guard present after every worker exited. The primary-death behavior therefore remains unqualified and both failure causes are release blockers.
...
- [ ] (2026-08-22 15:20Z) Corrected one concrete cause of the Windows primary-death failure: guard release now drains an in-flight heartbeat file-time update before unlinking its token-owned guard, avoiding a Windows open-handle race. The real primary-death matrix, focused cluster suite, complete 1,938-test concurrent suite, and configured four-process suite pass locally; hosted Windows confirmation remains required. The still-opaque held-backup 400 now reports its response body and hook error/stack so the next Windows run exposes the actual failure instead of encouraging a retry-based workaround.
...
Milestone 10 first production-smoke and Windows-correction batch:
    ...
    Guard correction: release clears the heartbeat timer and drains its serialized
                      in-flight utimes operation before token-checking and unlinking
    ...
    bun run lint: 0 warnings, 0 errors
    git diff --check: passed
    Hosted Windows correction/diagnostic qualification: pending
