...
- [ ] After that correctness freeze, run the matched full PocketBun benchmark suite with one, two, and four workers on the reference host, compare it with the recorded PocketBase v0.40 baseline, and update the repository's public benchmark results without selectively dropping regressions. The first exact-`8048583e` attempt stopped after its first one-worker run because the committed source archive omitted the upstream JavaScript benchmark hook fixture: 147 scenarios passed, while the three `/js` scenarios correctly failed validation with 404. The repository-wide `pb_hooks/` ignore rule now exempts this one vendored fixture; restart all five runs per worker count from a committed source archive and retain the same 150-row, zero-error, worker-metadata, and source-provenance validation.
