...
- [ ] Run one directional PocketBase v0.40 benchmark on the rebooted two-vCPU host using the exact upstream benchmark revision `05625dc2` that completed on the big host. The failed small-host runs accidentally used PocketBun's stale vendored benchmark snapshot with one `http.Transport` per request, whereas the big-host launcher synced upstream first and used its shared global client. One run is enough for this curiosity; do not expand it into another five-run release baseline or present it as directly comparable with the historical old-client numbers.
...
- [ ] Run a two-worker, two-hour steady-state Linux memory soak on the final current revision with fixed worker PIDs: no deliberate worker deaths, full-cluster restarts, or forced garbage collection. Keep CRUD/SQLite, realtime, exact shared-rate-limit, checkpoint, and live-backup work active; sample every process by PID and compare post-warm-up RSS medians/slopes so worker retention is not hidden by process replacement. A one-minute rehearsal passed. The first two-hour attempt stayed healthy for about 30 minutes, then the harness incorrectly treated PocketBase's intentional 30-minute SSE maximum lifetime as a server failure; rotate only the clients at 25 minutes while keeping worker PIDs fixed, and rerun from the beginning after the production correctness changes are committed.
- [ ] Run the final current revision through the final 60-minute four-worker soak. The `ebd9bd4d` run remains useful fault evidence but no longer qualifies the exact release revision after the expiring-state and IPC-backpressure corrections; rerun from the beginning, then complete the no-known-correctness-failure release review.
- [ ] After that correctness freeze, run the matched full PocketBun benchmark suite with one, two, and four workers on the reference host, compare it with the recorded PocketBase v0.40 baseline, and update the repository's public benchmark results without selectively dropping regressions.
...
Milestone 10 remains the production-release gate, but most of its evidence is now complete. Hosted Ubuntu, macOS, and Windows runs qualify leader and follower startup failures, transaction rollback, pending and lost coordinator responses, primary death, protocol violations, crash-budget exhaustion, SQLite contention, backup ownership, restart, supported-platform restore, SSE-owner replacement, and adversarial cross-client realtime isolation. Windows built-in restore is intentionally unsupported exactly like PocketBase v0.40 and its unchanged Admin UI. Stateful one-hour Linux soaks passed with two and four workers, and later v0.40 two-worker and four-worker five-hour soaks completed every functional and resource-integrity assertion. Worker RSS and descriptors remained stable; modest primary RSS growth remains a monitoring observation, not evidence of a serious or operationally unbounded leak. PocketBase v0.40's live-backup boundary is ported and cluster-qualified, including distinct owner/deleter/uploader processes and exact archive file inclusion. Read scaling reached +65.7% across two physical Linux cores, the DB-free control reached +94.6%, and the local macOS proxy topology scaled monotonically through eight workers. Primary-pressure testing then found an individual-IPC rate-limit bottleneck; ordered batching raised the four-worker path from 1,238 to 27,980 req/s while preserving exact primary-owned decisions. The matched realtime run delivered all 672,320 expected events exactly and saturated workers before the primary. Production documentation and the post-batching local repository, package, docs, mapping, Admin UI, and current hosted CI gates are complete.

Release is now blocked only on the current-revision steady-state two-hour memory soak, the final four-worker one-hour fault soak, and the no-known-correctness-failure review. Hosted Ubuntu, macOS, Windows, and E2E qualification of the current per-slot crash-budget revision has passed. The steady-state soak complements rather than replaces the earlier chaos soaks: stable PIDs make per-worker retention visible without recovery resetting process memory. The public PocketBase/PocketBun benchmark matrix remains intentionally deferred until those correctness gates pass; the additional same-class PocketBase v0.40 baseline may run independently because it changes no PocketBun code. Windows built-in restore is an accepted compatibility exclusion, and the modest primary-RSS trend is a monitoring note rather than separate implementation work. No unexplained flaky result may be waived by adding a default retry.
