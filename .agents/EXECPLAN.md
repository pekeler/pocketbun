...
- [ ] Finish ordered realtime-presence qualification on exact commit `a2776b42`: the deterministic ordering regression and complete local gate passed. A four-worker accelerated restart stress survived about 50 controlled restarts in ten minutes; fresh two-worker and four-worker fault soaks then ran for 60 minutes, completed 63,180 and 57,518 shared-SQLite cycles, exercised worker replacement and controlled restart, kept every shared-rate assertion exact, passed final backup/integrity checks, and exited cleanly without a missing steady-state or controlled-restart event. Hosted CI remains pending because the commit has not been pushed.
...
- [ ] Measure whether three workers outperform four on the four-vCPU/two-physical-core host. First run the identical externally driven read workload with two, three, and four workers allowed to use the whole host, recording throughput, latency, primary/worker CPU, RSS, and per-worker request shares. This is the production-realistic worker-count choice: Linux may schedule every process and helper thread on every CPU, so merely starting three workers does not reserve one vCPU for the primary. Add an affinity control only if the whole-host result is ambiguous. If three workers win materially, recommend fewer workers than reported vCPUs on SMT hosts instead of assuming one worker per vCPU.
...
Revision note, 2026-08-26 / Codex: Added a production-realistic two/three/four-worker comparison on the four-vCPU host before finalizing worker-count guidance. Three workers could beat four by leaving the Linux scheduler more room for the primary and Bun native helper threads, but an unpinned process count does not literally reserve a CPU. Reuse the existing external read workload and process sampler first; add a pinned control only if the result needs attribution.
