...
- [x] (2026-08-28) Upgraded PocketBun to PocketBase v0.40.1 without committing. Synced upstream commit `bc8ffed4` and the Admin UI, preserved omitted same-name OAuth2 provider fields, pinned Bun's native malformed-UTF-8 replacement across the upstream serialization surfaces, aligned version/changelog metadata, passed 1,975 tests plus every local repository/package/UI gate, and completed a clean Ponytail complexity review.

## Surprises & Discoveries

- Observation: PocketBase v0.40.1's invalid-UTF-8 serializer changes require regression coverage but no PocketBun production serializer.
  Evidence: JavaScript strings cannot retain invalid UTF-8 byte sequences. Bun's standard `TextDecoder` converts the upstream test bytes to U+FFFD before router, picker, record, `JSONArray`, `JSONMap`, or `JSONRaw` serialization; focused ports of all those upstream cases pass without a new helper. The only functional compatibility defect was the collection options merge replacing omitted OAuth2 provider fields.
...
- Decision: implement v0.40.1's OAuth2 correction in the existing `mergeOAuth2Config` collection-decode boundary and rely on Bun's native byte-to-string replacement for malformed UTF-8.
  Rationale: every REST, model-load, and JSVM collection update already crosses this one merge function, so matching providers by name there preserves omitted fields without a second config abstraction. JavaScript has no invalid-UTF-8 string state after byte decoding, and the ported upstream tests prove the standard Web API behavior; a custom JSON serializer would duplicate the platform and add risk.
  Date/Author: 2026-08-28 / Codex, following Ponytail review
...
Milestone 13 is complete locally and remains intentionally uncommitted for repository-owner review. PocketBun now targets PocketBase v0.40.1 at upstream commit `bc8ffed4`, serves the refreshed vendored Admin UI, and preserves omitted OAuth2 provider credentials and other settings when the submitted provider name is unchanged. Bun's native malformed-byte decoding matches the upstream JSON replacement behavior without production code. Focused tests passed 150 cases; the complete suite passed 1,975 tests; source and package typechecks/build, lint, formatting, version alignment, generated docs, upstream mapping, whitespace checks, and four Admin UI E2E checks all passed. Ponytail's final over-engineering review reported `Lean already. Ship.` and required no iteration.
...
### Milestone 13: upgrade compatibility to PocketBase v0.40.1

Change `pocketbase_tag.txt` to `v0.40.1` and `package.json` to `0.40.1-pocketbun.0`, then run `bun run upstream:sync` so `.upstream/pocketbase` and `vendor/pocketbase-admin-ui/dist` come from upstream commit `bc8ffed4`. Compare every upstream file changed since v0.40.0 with its PocketBun counterpart. In `src/core/collection_model.ts`, update the existing `mergeOAuth2Config` boundary so submitted providers replace the provider list but omitted fields are copied from the existing provider with the same name. Port the upstream configuration and collection endpoint regressions into the corresponding colocated TypeScript tests. Verify malformed UTF-8 bytes through Bun's native `TextDecoder`, router JSON, picker, record, and custom JSON-type paths; add only the smallest regression coverage needed because JavaScript strings cannot contain invalid UTF-8 byte sequences after decoding. Update the top-level `Unreleased` changelog with the v0.40.1 compatibility result and exact upstream commit.

Run focused tests first, then the mandatory `bun run format:fix`, `bun test --concurrent`, `bun run typecheck`, and `bun run lint` commands. Also run version alignment, package type/build, generated-doc, upstream-mapping, whitespace, and Admin UI checks when they are affected. Review the final uncommitted diff using the Ponytail over-engineering rubric; apply only findings that make the implementation smaller without weakening upstream behavior or regression coverage. Do not create a commit.
...
Revision note, 2026-08-28 / Codex: Removed the duplicated CLI/programmatic cluster role-selection block after committing the functional fix. Both entrypoints now call one internal `runClusterEntrypoint()` helper that attaches a forked worker or runs the existing primary supervisor; public-boundary validation and the CLI-only serve-command check remain with their callers. All 11 real-process cluster lifecycle tests, typecheck, formatting, lint, and whitespace checks pass.

Revision note, 2026-08-28 / Codex: Completed the uncommitted PocketBase v0.40.1 upgrade. Synced upstream commit `bc8ffed4` and Admin UI assets, fixed same-name OAuth2 provider merges at the existing collection decode boundary, and ported the upstream malformed-UTF-8 cases using Bun's native decoding behavior. Focused and complete tests, all repository/package/docs/UI gates, and the requested Ponytail complexity review pass; no commit was created.
