...
describe("otp queries", () => {
  it("FindAllOTPsByRecord", async () => {
    ...
    try {
      ...
      const scenarios = [
        ...
        { record: superuser2, expected: ["superuser2_0", "superuser2_1", "superuser2_3", "superuser2_2", "superuser2_4"] },
        { record: superuser4, expected: [] },
        { record: user1, expected: ["user1_0"] },
      ];

      for (const scenario of scenarios) {
        const result = app.FindAllOTPsByRecord(scenario.record);
  ...
  it("FindAllOTPsByCollection", async () => {
    ...
    try {
      ...
      for (const scenario of scenarios) {
        const result = app.FindAllOTPsByCollection(scenario.collection);
        expect(result.length).toBe(scenario.expected.length);
        for (const [index, id] of scenario.expected.entries()) {
          expect(result[index]?.Id).toBe(id);
        }
  ...
  it("FindOTPById", async () => {
    ...
    try {
      ...
      for (const scenario of scenarios) {
        ...
        let resultId = "";
        try {
          const result = app.FindOTPById(scenario.id);
          resultId = result.Id;
        } catch (error) {
          err = error as Error;
        }
  ...
  it("DeleteAllOTPsByRecord", async () => {
