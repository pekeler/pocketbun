...
describe("mfa queries", () => {
  it("FindAllMFAsByRecord", async () => {
    ...
    try {
      const stubErr = await StubMFARecords(app);
    ...
    try {
      ...
      for (const scenario of scenarios) {
        ...
        try {
          ...
          const err = await app.DeleteAllMFAsByRecord(scenario.record);
          expect(err).toBeNull();
  ...
  it("DeleteExpiredMFAs", async () => {
    const checkDeletedIds = async (
      ...
      try {
        ...
        const err = await app.DeleteExpiredMFAs();
        expect(err).toBeNull();
    ...
    await checkDeletedIds(["user1_0", "superuser2_1", "superuser2_2", "superuser2_4", "superuser3_1"], async (app) => {
      const superusers = app.findCollectionByNameOrId(CollectionNameSuperusers);
      if (!superusers) {
