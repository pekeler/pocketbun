...
| Windows `Bun.spawnSync` intermittent empty/invalid stdout | https://github.com/oven-sh/bun/issues/27482 | closed; reopening requested for Bun `1.4.0` regression | Bun `1.4.0` Windows CI reproduced an empty stdout result after the issue was considered fixed, and a second run showed that redirecting child stdout to a `Bun.file(...)` can also leave the file empty. The synchronous JSVM HTTP child now writes each result directly to a private temporary file because retrying a completed mutating request could duplicate side effects. |
