...
| Playwright test runner under Bun fails on `.esm.preflight` resolution | https://github.com/oven-sh/bun/issues/28609 | open | Despite the Bun 1.4 release announcement, local Bun `1.4.0` still fails `bun --bun playwright test` before loading PocketBun's config. Keep `bun run e2e`, which launches the installed Playwright runner with its normal Node runtime, until the upstream issue is fixed. |
...
| Windows `Bun.spawnSync` intermittent/early completion | https://github.com/oven-sh/bun/issues/27482 | closed; reopening requested for Bun `1.4.0` regression | Three Bun `1.4.0` Windows CI runs returned exit code zero while successively losing piped stdout, leaving redirected `Bun.file(...)` output empty, and returning before a child-created result file existed. The synchronous JSVM HTTP client now launches one asynchronous child and synchronously waits for its atomically published private result file; it never retries a possibly completed mutating request. |
...
gh api repos/oven-sh/bun/issues/28609 --jq '[.number, .state, .title, .html_url] | @tsv'
