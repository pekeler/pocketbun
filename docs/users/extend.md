...
To update older hooks and migrations automatically, run `pocketbun server-js upgrade-source`. It scans `./pb_hooks` and `./pb_migrations` by default; use `pocketbun server-js upgrade-source --check` in CI to fail when deprecated aliases remain, and review the generated diff before committing. The fixer rewrites deprecated uppercase server-side JavaScript member access and known PocketBun option-object keys to their lower camelCase equivalents, including app methods such as `RunInTransaction`, `CreateBackup`, `RecordQuery`, and `Restart`; record, `DateTime`, form, `ApiError`, `ValidationError`, `RequestInfo`, `Cookie`, `Command`, `SubscriptionMessage`, `Context`, field option, and hook handler names. It leaves unrelated application data keys, such as HTTP headers and `globalThis.secrets.*`, unchanged. It also updates released PocketBun package aliases like `RegisterHooksPlugin*` / `RegisterJSVM*`, `RegisterServerJS*`, `TemplateLangJS`, and PascalCase package config keys when you pass files that contain package setup code.
...
Use `onBootstrapClear` to release resources allocated during `onBootstrap`. It runs before shutdown or reinitialization, and when you call `await $app.clearBootstrap()`. In custom Bun entrypoints, await `app.clearBootstrap()` and use `await app.bootstrapAsync()` when reinitializing so asynchronous hooks and queued log writes finish before databases reopen. The older `app.resetBootstrapState()` is a deprecated alias with the same awaitable cleanup behavior.
