...
- Record create/update hot paths now avoid some unnecessary hook-bridge work, including more no-op model-to-record bridge triggers, duplicate request-body parsing, per-record interceptor filtering, repeated request-hook lookups, repeated untagged model-hook wrapping, unstable rule-query placeholder generation, extra cached-collection scans, repeated route score recomputation, and internal request/record access churn, reducing measured write overhead on common JSON CRUD requests and improving rule-heavy create benchmarks.
- Create-heavy API requests now skip disabled activity-logging work earlier, avoid eager remote-address and URL setup on the normal `serve()` path, and do less route path parsing overhead, trimming measured request-layer cost on local write benchmarks.
...
- Added maintainer profiling commands for Bun CPU/heap markdown reports, scenario-shaped heap profiles, and targeted `node:inspector` request CPU profiles, including an external load generator for cleaner server-only samples.
...
- The custom-route and SSR docs now point Bun-native users at `Bun.CSRF` as a practical building block when they need to add cookie-backed endpoints or server-rendered forms on top of PocketBun.
