...
- Fixed remaining cross-platform CI flakes by improving multipart fallback body candidate handling, parsing Event multipart bodies directly from the request to avoid clone boundary issues on Windows, loosening uploaded-file detection to accept Bun file-like values across runtimes, hardening sync HTTP helper subprocess output handling/retries, relaxing tight JSVM timing assertions, extending slow test timeouts, and running Linux/macOS CI tests with a `10s` per-test timeout ceiling.
