...
- Further stabilized cross-platform CI by normalizing static path checks for backslash traversal patterns, improving multipart form fallback parsing when boundary headers are missing/mismatched, and making test temp-dir cleanup best-effort when Windows file locks persist.
- Further hardened flaky Windows CI paths by adding sync-fetch stdout flush/retry safeguards, using deterministic multipart request construction in router event tests, and increasing the Windows test timeout ceiling to 30s.
- Fixed remaining cross-platform CI flakes by improving multipart fallback body candidate handling, restoring deterministic multipart request fixtures in Event tests, loosening uploaded-file detection to accept Bun file-like values across runtimes, hardening sync HTTP helper subprocess output handling/retries (including transient connection failures), relaxing tight JSVM timing assertions, extending slow test timeouts, and running Linux/macOS CI tests with a `10s` per-test timeout ceiling.
